...
// Runs at build time (see package.json's "build"/"predev" scripts) so the
// server can just `res.sendFile` a pre-rendered `public/index.html` on
// every `GET /` request, instead of re-parsing README.md with `marked` on
// every request.
const markdown = readFileSync(path.resolve('README.md'), 'utf-8');
...
writeFileSync(path.resolve('public', 'index.html'), html);

console.log('Generated public/index.html from README.md');
